...
- 브라우저에 쿠키를 저장하는 장소가 따로 존재
- 쿠키로 줄 수 있는 데이터가 한정적이라 컴퓨터를 망가뜨릴 수 있는 정보는 아님
...
- HttpOnly : 스크립트의 쿠키 접근 가능 여부 결정 (XSS 공격 방어)
  ...
  - HttpOnly가 false일 때 : JavaScript 접근 가능

  - HttpOnly가 true일 때 : JavaScript 접근 불가능
...
- SameSite : CORS 요청의 경우 옵션 및 메서드에 따라 쿠키 전송 여부 결정
  ...
  - None : 모든 메소드 요청에 대한 쿠키 전송 가능 (Secure 옵션을 통해 HTTPS 사용이 전제되어야 함)
...
- 세션 아이디는 DB에 저장하지 않고 로그아웃시 사라지며, 세션스토어의 정보고 삭제
